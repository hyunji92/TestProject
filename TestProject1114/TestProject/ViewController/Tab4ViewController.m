...
-(void) displayScrollView{
    ...
    for (NSInteger i = 0;  i < self.items.count; i++) {
        ...
        NSArray *nibs = [[NSBundle mainBundle] loadNibNamed:@"ViewinScroll" owner:self options:nil]; // view 불러와서 밑에 생성
        ViewinScroll *subview = (ViewinScroll *)[nibs firstObject];
        subview.btn.tag = i;
        
//        UIImageView *iv = [subview viewWithTag:7];
//        UILabel *label = [subview viewWithTag:8];
//        UIButton *btn  = [subview viewWithTag:9];
 
        [subview.btn addTarget:self action:@selector(scrollItemClick:) forControlEvents:UIControlEventTouchUpInside];
        
        CGRect rect = CGRectMake(i*70, 0, 70, 70);
        [subview setFrame:rect];
        [_scrollView addSubview:subview];
        
        subview.image.image = [UIImage imageWithData:[dic objectForKey:@"img"]];
        subview.label.text = [dic objectForKey:@"title"];
        

    }
    
    [_scrollView setContentSize:CGSizeMake(self.items.count * 70, 70)];
}

-(IBAction)scrollItemClick:(id)sender{
    
    NSInteger tag = ((UIButton *)sender).tag;
    NSLog(@"items click!!! %ld" , tag);
}
