...
//        }
    ...
    private int calculateScrollForSelectingPosition(final int itemPosition, final RecyclerView.State state) {
        ...
        return VERTICAL == mOrientation ? fixedItemPosition * (int)(mDecoratedChildHeight * mViewPostLayout.getScale()) : fixedItemPosition * (int)(mDecoratedChildWidth * mViewPostLayout.getScale());
    }
    ...
    private void fillDataVertical(final RecyclerView.Recycler recycler, final int width, final int height, final boolean childMeasuringNeeded) {
        final int start = (width - (int)(mDecoratedChildWidth * mViewPostLayout.getScale())) / 2;
        final int end = start + (int)(mDecoratedChildWidth * mViewPostLayout.getScale());

        final int centerViewTop = (height - (int)(mDecoratedChildHeight * mViewPostLayout.getScale())) / 2;
        ...
        for (int i = 0, count = mLayoutHelper.mLayoutOrder.length; i < count; ++i) {
            ...
            final int bottom = top + (int)(mDecoratedChildHeight * mViewPostLayout.getScale());
    ...
    private void fillDataHorizontal(final RecyclerView.Recycler recycler, final int width, final int height, final boolean childMeasuringNeeded) {
        final int top = (height - (int)(mDecoratedChildHeight * mViewPostLayout.getScale())) / 2;
        final int bottom = top + (int)(mDecoratedChildHeight * mViewPostLayout.getScale());

        final int centerViewStart = (width - (int)(mDecoratedChildWidth * mViewPostLayout.getScale())) / 2;
    ...
    private void generateLayoutOrder(final float currentScrollPosition, @NonNull final RecyclerView.State state) {
        ...
        if (mCircleLayout && 1 < mItemsCount) {
            ...
            final int firstVisible = 0;
            final int lastVisible = mItemsCount - 1;
    ...
    protected int getCardOffsetByPositionDiff(final float itemPositionDiff) {
        ...
        final int dimenDiff;
        if (VERTICAL == mOrientation) {
            dimenDiff = (getHeightNoPadding() - (int)(mDecoratedChildHeight * mViewPostLayout.getScale())) / 2;
        } else {
            dimenDiff = mDecoratedChildWidth + mItemPadding;
        }

        //noinspection NumericCastThatLosesPrecision
        return (int) Math.round(Math.signum(itemPositionDiff) * dimenDiff * smoothPosition * (mViewPostLayout.getScale() * mViewPostLayout.getScale()));
    }
    ...
    protected double convertItemPositionDiffToSmoothPositionDiff(final float itemPositionDiff) {
        ...
        return StrictMath.pow(absIemPositionDiff / mLayoutHelper.mMaxVisibleItems, 1 / 1.2f);
...
//            return StrictMath.pow(absIemPositionDiff, 1.2f);
//        }
    }
    ...
    protected int getScrollItemSize() {
        if (VERTICAL == mOrientation) {
            return (int)(mDecoratedChildHeight * mViewPostLayout.getScale());
        } else {
            return (int)(mDecoratedChildWidth * mViewPostLayout.getScale());
        }
    }
